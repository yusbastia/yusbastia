# $id rssnews.tcl - RSS news announcer eggdrop 1.6.x demond@demond.net Exp
#                                                                     
...
set feeds(Google:#oops) {20 https://news.google.com/rss?hl=id&gl=ID&ceid=ID:id}
set feeds(Antara:#i) {20 https://www.antaranews.com/rss/terkini.xml}
set feeds(GPost:#squad) {25 https://gorontalo.tribunnews.com/rss}
	...
	foreach {foo item} [regexp -all -inline {(?i)<item.*?>(.*?)</item>} $data] {
		...
		if {[lsearch -exact $hash($chanfeed) [md5 $title]] == -1 && [botonchan $chan]} {
			if {![info exists header]} {
				...
				putlog "New feed(s) from $header. Announcing to channel..."
			}
			if {$count < $maxnew} {
				#puthelp "privmsg $chan :($idx) $title"
				puthelp "PRIVMSG $chan :2 \002$title\002 - $descr  14$link "
				incr count
			} {
				lappend indices $idx
			}
