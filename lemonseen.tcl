...
##                                                               ##
##  File Name: seen.tCL date: 06 November 2004                   ##
##  Has Been Ripper by MoBi aka PUPEN                            ##
##  Email: mobi@internux.web.id                                  ##
###################################################################
putlog "## ..:: Initializing Seen TCL ::... ##"
...
set bs(quiet_chan) "#yobayat #radio #kotamobagu #dj"
...
putlog "$bs(version):  -- MoBi Seen loaded --"
...
proc bs_dccreq {hand idx args no} {
  ...
  if {[lrange [split $output] 1 4] == "Gue enggak pernah liat"} {putallbots "bs_botsearch 0 $idx $hand $args"}
  putdcc $idx "$output" ; return $bs(logqueries)
}
...
proc bs_msgreq {nick uhost hand args no} { 
  ...
  if {$output == 0} {return 0}
  if {[lrange [split $output] 1 4] == "Gue enggak pernah list"} {putallbots "bs_botsearch 1 \{notice $nick\} $nick $args"}
  puthelp "notice $nick :$output" ; return $bs(logqueries)
}
...
proc bs_pubreq {nick uhost hand chan args no} {
  ...
  if {[string tolower $nick] == [string tolower $data] } {puthelp "$target :15\[seen4!15\] $nick, ngaca doms ngaca..." ; return $bs(logqueries)}
  if {[string tolower $data] == [string tolower $botnick] } {puthelp "$target :15\[seen4!15\] $nick, saya ada disini. kenapa kangen yaa??" ; return $bs(logqueries)}
  if {[onchan $data $chan]} {puthelp "$target :15\[seen4!15\] $nick, $data ada tuh disini!" ; return $bs(logqueries)}
  set output [bs_output $chan $nick $data $no] ; if {$output == 0} {return 0}
  if {[lrange [split $output] 1 4] == "Gue enggak pernah liat"} {putallbots "bs_botsearch 1 \{$target\} $nick $args"}
  puthelp "$target :$output" ; return $bs(logqueries)
}
proc bs_output {chan nick data no} {
  ...
  if {$data == ""} {return 0}
  if {[string tolower $nick] == $data} {return [concat $nick, elo khan ada disini sekarang.]}
  ...
  if {$no == 0} {
    ...
    if {$matches != ""} {
      ...
      if {$len > 99} {return [concat Ada $len yang cocok di database, tolong sebutkan permintaan anda lebih spesifik.]}
      set matches [bs_sort $matches]
      set key [lindex $matches 0]
      if {[string tolower $key] == [string tolower $data]} {return [bs_search $chan $key]}
      if {$len <= 5} {
        set output [ concat Ada $len orang yang cocok nih (sorted): [join $matches].]
        set output [concat $output  [bs_search $chan $key]] ; return $output
      } {
        set output [ concat Ada $len orang yang cocok nih. Ini 5 orang yang terakhir (sorted): [join [lrange $matches 0 4]].]
        set output [concat $output  [bs_search $chan $key]] ; return $output
      }
    }
  }
  ...
  if {$temp != 0} { return $temp } {
    ...
    if {![validuser [bs_filt $data]] || [string trimleft [lindex $version 1] 0]<1030000} { 
      return " $nick, Gue enggak pernah liat $data."
    } {
      set seen [getuser $data laston]
      if {[getuser $data laston] == ""} {return "$nick, Gue enggak pernah liat $data."}
      ...
      return [concat $nick, $data terakhir gue liat dia ada di $chan [bs_when [lindex $seen 0]] yang lalu.]
    }
  }
}
...
#bugfix: added a join on the $n  (c)
	...
	quit { set output [concat $n ($addy) terakhir gue liat dia keluar IRC dari $chan [bs_when $time] yang lalu. Pesan : ([join [lrange $data 5 e]]).] }
	kick { set output [concat $n ($addy) terakhir gue liat dia di tendang dari $chan sama [lindex $data 5] [bs_when $time] yang lalu dengan pesan ([join [lrange $data 6 e]]).] }
	rnck {
	  set output [concat $n ($addy) terakhir gue liat dia ganti nick dari [lindex $data 5] di [lindex $data 4] [bs_when $time] yang lalu.] 
	  if {[validchan [lindex $data 4]]} {
	    if {[onchan $n [lindex $data 4]]} {
	      set output [concat $output $n masih ada disini sekarang.]
	    } {
	      set output [concat $output  Sekarang gue enggak liat $n tuh.]
	    }
	  }
	}
	nick { 
	  set output [concat $n ($addy) terakhir gue liat dia ganti nick ke [lindex $data 5] di [lindex $data 4] [bs_when $time] yang lalu.] 
	}
	splt { set output [concat $n ($addy) terakhir gue liat dia keluar $chan karena split server [bs_when $time] yang lalu.] }
	rejn { 
	  set output [concat $n ($addy) terakhir gue liat dia join $chan sesudah terkena split server [bs_when $time] yang lalu.] 
	...
	join { 
	  set output [concat $n ($addy) terakhir gue liat dia join $chan [bs_when $time] yang lalu.]
	  if {[validchan $chan]} {if {[onchan $n $chan]} {set output [concat $output  $n masih ada di $chan.]} {set output [concat $output  Gue enggak liat $n di $chan sekarang tuh.]}}
	}
	away {
	  set reason [lrange $data 5 e]
	    ...
	    set output [concat $n ($addy) terakhir gue liat di away ($reason) di $chan [bs_when $time] yang lalu.]
	  }
	}
	chon { 
...
proc bs_seenmask {ch nick args} {
  ...
  if {$chan != "" && [string trimleft $chan #] != $chan} {
    if {![validchan $chan]} {return "Gue enggak pernah join $chan."} {set chan [string tolower $chan]}
  } { set $chan "" }
...
#bugfix: unnecessarily complex masks essentially freeze the bot
  ...
  if {$len > 99} {return "Ada $len data yang cocok di database, tolong sebutkan permintaan anda lebih spesifik."}
  set matches [bs_sort $matches]
  if {$len <= 5} {
    set output [concat Ada $len orang yang cocok (sorted): [join $matches].]
  } {
    set output "Ada $len orang yang cocok. Ini 5 orang yang terakhir (sorted): [join [lrange $matches 0 4]]."
  }
...
### END OF SCRIPT
putlog "seen.tcl By MoBi ® Succesfully LoaDeD..."
