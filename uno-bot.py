...
class UnoGame:
    def __init__(self, bot, channel):
        ...
        # Memori untuk membaca kartu lawan
    ...
    def _schedule_bot_move(self):
        with self.lock:
            ...
            if UNO_DEBUG:
                print(f"[DEBUG] bot move scheduled for {self.robot_nick}")
    ...
    # ========== PERBAIKAN: SETIAP GILIRAN NICK DISEBUT ==========
    def _start_turn(self):
        with self.lock:
            player = self._current_player()
            symbol = random.choice(self.turn_symbols)
            self.unomsg(f"{symbol} {BOLD}Giliran{BOLD} {symbol} {color_nick(player.nick)}")
            self.is_draw = False
            self._reset_skip_timer()
    ...
    def _evaluate_card(self, card):
        ...
        if ct == 'number':
            ...
            score += points * 2
            if any(opp.hand_size() <= 2 for opp in opponents_near_win):
                score += 100000
    ...
    def _bot_pick_color(self, card):
        ...
        if max(my_colors.values()) > 0:
            best_colors = [col for col, cnt in my_colors.items() if cnt == max(my_colors.values())]
            return random.choice(best_colors)
        
        opponent_fav_colors = {'R':0, 'G':0, 'B':0, 'Y':0}
        for p in self.players:
            if p.nick != self.robot_nick:
                fav = self._predict_opponent_hand(p.nick)
                for col in opponent_fav_colors:
                    opponent_fav_colors[col] += fav.get(col, 1)
        
        if sum(opponent_fav_colors.values()) > 0:
            return min(opponent_fav_colors, key=opponent_fav_colors.get)
    ...
    def _robot_play(self):
        with self.lock:
            if UNO_DEBUG:
                print(f"[DEBUG] _robot_play dipanggil")
            if self.mode != 2 or self.paused:
                return
            player = self._current_player()
            if player.nick != self.robot_nick:
            ...
            if self.is_draw:
                card_index = self._bot_choose_card()
    ...
    def color_change(self, nick, color):
        with self.lock:
            if self.mode != 2 or self.paused:
                return
            if not self.is_color_change or nick != self.color_picker:
                return
            player = self._get_player(nick)
            if player:
                player.idle_streak = 0
    ...
    def show_help(self, nick):
        ...
        self.send_private(nick, f"{BOLD}Admin:{RESET} .pause, .play, .resetpoint, .reload, .aggressive, .autokick, .difficulty, .join, .part")
    ...
    def show_version(self):
        ...
        self.unomsg(f"{BOLD}{COLOR_BLUE}╚════════════════════════════════╝{RESET}")
        self.unomsg(f"{COLOR_GREEN}Fitur: mode tim, multi-channel, skor terpisah, bot cerdas & agresif, Blackjack bonus, tampilkan kartu pemenang hanya untuk bot, larangan draw jika memiliki Wild Draw Four valid, auto-kick (2 idle), tingkat kesulitan (hard/medium/easy), variasi simbol giliran acak, tampilkan kartu pemain yang kalah, bot pilih warna berdasarkan kartu sendiri, memori untuk membaca kartu lawan, dan nick pemain disebut di setiap giliran{RESET}")
    ...
    def _win(self, winner_nick, winning_card=None):
        with self.lock:
            ...
            if self.team_mode:
                ...
                total_winner_points = winner_points + bonus_streak

                if winner_points == 21:
                    blackjack_bonus = 2100
                    total_winner_points += blackjack_bonus
                    self.unomsg(f"{BOLD}{COLOR_GREEN}🎰 BLACKJACK! Tim {winning_team.name} mendapatkan bonus {blackjack_bonus} {UNO_POINTS_NAME}!{RESET}")
                ...
                self.unomsg(f"  {COLOR_GREEN}➤{RESET} Mendapat {total_winner_points} {UNO_POINTS_NAME}.")

                if winner_nick != self.robot_nick:
                    update_score(self.channel, winner_nick, total_winner_points)
                for team in losing_teams:
                    for p in team.members:
                        if p.nick != self.robot_nick:
                            ...
                            self.unomsg(f"  Kartu yang dipegang: {uno_cardcolorall(p.hand)}")
    ...
    def _win_normal(self, winner_nick, winning_card=None):
        with self.lock:
            ...
            if winner_points == 21:
                blackjack_bonus = 2100
                total_winner_points += blackjack_bonus
                self.unomsg(f"{BOLD}{COLOR_GREEN}🎰 BLACKJACK! {color_nick(winner_nick)} mendapatkan bonus {blackjack_bonus} {UNO_POINTS_NAME}!{RESET}")
            ...
            for nick, val in loser_values.items():
                if nick != self.robot_nick:
                    ...
                    player = self._get_player(nick)
                    if player:
                        self.unomsg(f"  Kartu yang dipegang: {uno_cardcolorall(player.hand)}")
...
def handle_pubmsg(bot, connection, nick, channel, message):
    ...
    if message.startswith('.'):
        ...
        if not parts:  # PENCEGAHAN ERROR: jika perintah kosong
            return
        cmd = parts[0].lower()
        args = parts[1:]
        ...
        if cmd == 'autokick':
            ...
            if args and args[0].lower() in ('on', 'off'):
                ...
                connection.privmsg(channel, f"{UNO_LOGO} Auto-kick {state} oleh {nick}.")
            else:
                connection.notice(nick, "Gunakan .autokick on atau .autokick off")
            return
        ...
        if cmd == 'difficulty':
            ...
            if args and args[0].lower() in ('hard', 'medium', 'easy'):
                ...
                connection.privmsg(channel, f"{UNO_LOGO} Tingkat kesulitan bot diubah menjadi {game.difficulty.upper()} oleh {nick}.")
            else:
        ...
        if cmd == 'version':
            if game:
                ...
                connection.privmsg(channel, f"{UNO_LOGO} Uno Game+ mode tim + multi-channel")
            return
        ...
        if cmd in ('cmds', 'help'):
            if game:
                ...
                connection.notice(nick, "Perintah UNO: .uno .stop .top10 .won [nick] .version .row .pause (admin) .play (admin) .resetpoint (admin) .reload (admin) .aggressive (admin) .autokick (admin) .difficulty (admin) .join #channel (admin) .part #channel (admin) .remove [nick] .help .cmds. Mode tim: .unotim, .team ...")
            return
