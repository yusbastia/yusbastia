...
UNO_MAX_PLAYERS = 10
UNO_AUTO_SKIP_PERIOD = 30          # detik timeout pemain (idle)
...
UNO_DEBUG = 0                       # 1 = tampilkan log debug (opsional)
...
class UnoGame:
    def __init__(self, bot, channel):
        ...
        # MODE AGRESIF DI AKTIFKAN SECARA DEFAULT
    ...
    def _schedule_bot_move(self):
        with self.lock:
            ...
            if UNO_DEBUG: print(f"[DEBUG] bot move scheduled for {self.robot_nick}")
    ...
    def stop(self, nick="console"):
        with self.lock:
            ...
            self.teams = []
            self.team_mode = False
            self.msg(f"{COLOR_RED}⏹️ Game dihentikan oleh {nick}.{RESET}")
    ...
    def pause_game(self, nick):
        with self.lock:
            ...
            if not self.paused:
                ...
                self.msg(f"{COLOR_ORANGE}⏸️ Game dijeda oleh {nick}.{RESET}")
            else:
    ...
    def resume_game(self, nick):
        with self.lock:
            ...
            if self.paused:
                ...
                self.msg(f"{COLOR_GREEN}▶️ Game dilanjutkan oleh {nick}.{RESET}")
            else:
    ...
    def team_start_mode(self, nick):
        with self.lock:
            ...
            self.discard = []
            self.start_time = int(time.time())
            self.unomsg(f"{COLOR_CYAN}⚡ UNO TEAM ⚡{RESET}")
    ...
    def _begin_team_game(self):
        with self.lock:
            ...
            self.unomsg(f"{COLOR_YELLOW}➡️ Urutan giliran: {RESET}" + " → ".join(color_nick(p.nick) for p in self.players))
    ...
    def _start_turn(self):
        with self.lock:
            ...
            self.unomsg(f"{BOLD}{COLOR_YELLOW}⚡ GILIRAN ⚡ {RESET}{color_nick(player.nick)}")
            self.is_draw = False
            self._reset_skip_timer()
    ...
    def _auto_skip(self):
        with self.lock:
            if self.mode != 2 or self.paused:
                return
            player = self._current_player()
            # Langsung keluarkan pemain karena idle
            self.unomsg(f"{color_nick(player.nick)} telah idle selama {self.auto_skip_period} detik dan dikeluarkan dari permainan.")
            self.remove_player(player.nick)

    # ---------- Logika Bot (Pintar + Agresif) ----------
    ...
    def _evaluate_card(self, card):
        ...
        # Mode agresif selalu aktif
        if self._current_player().hand_size() == 1:
            score += 200000
        if self._current_player().hand_size() == 2:
            score += 10000
        ...
        for opp in opponents_near_win:
            if opp.hand_size() == 1:
                ...
                if ct in ('skip', 'reverse', 'draw2', 'draw4'):
                    score += 5000
        if ct in ('wild', 'draw4') and self._current_player().hand_size() <= 3:
            score += 2000
    ...
    def _bot_pick_color(self, card):
        """Pilih warna: jika bot tinggal 2 kartu dan kartu ini wild/draw4, pilih warna kartu lain; else pilih warna paling sedikit di tangan lawan."""
        player = self._current_player()
        if player.hand_size() == 2 and card_type(card) in ('wild', 'draw4'):
            other_cards = [c for c in player.hand if c != card]
            if other_cards:
                other_card = other_cards[0]
                col = card_color(other_card)
                if col in COLOR_NAMES:
                    return col
        ...
        if sum(opponent_colors.values()) > 0:
            return min(opponent_colors, key=opponent_colors.get)
    ...
    def play_card_internal(self, nick, card, from_bot=False):
        with self.lock:
            ...
            if ct == 'skip':
                ...
                self.unomsg(f"{color_nick(nick)} ➕2 {card_display(card)}")
            elif ct == 'draw4':
                self.unomsg(f"{color_nick(nick)} 💀💀💀 {card_display(card)}")
            elif ct == 'wild':
                self.unomsg(f"{color_nick(nick)} 🌈 {card_display(card)}")
    ...
    def draw_card(self, nick, from_bot=False):
        with self.lock:
            ...
            # Aturan Wild Draw Four: jika pemain memiliki kartu Wild Draw Four yang valid, tidak boleh draw
            for card in player.hand:
    ...
    def check_uno(self, player):
        if player.hand_size() == 1:
            self.unomsg(f"\001ACTION {color_nick(player.nick)} ⚠️ UNO! ⚠️\001")
    ...
    def show_top_card(self):
        self.unomsg(f"{BOLD}{COLOR_CYAN}🔝 KARTU ATAS 🔝{RESET} {card_display(self.top_card)}")
    ...
    def show_time(self):
        ...
        self.unomsg(f"{BOLD}{COLOR_CYAN}⏱️ Game time {duration(elapsed)}{RESET}")
    ...
    def show_stats(self):
        self.unomsg(f"{BOLD}{COLOR_CYAN}🃏 Played: {self.cards_played}{RESET}")

    def show_help(self, nick):
        self.send_private(nick, f"{BOLD}{COLOR_GREEN}📚 DAFTAR PERINTAH UNO 📚{RESET}")
        self.send_private(nick, f"{COLOR_YELLOW}────────────────────────────────{RESET}")
        self.send_private(nick, f"{BOLD}Game:{RESET} .uno, .unotim, .stop, .remove [nick]")
        self.send_private(nick, f"{BOLD}Info:{RESET} .top10, .won [nick], .row, .version, .cmds")
        self.send_private(nick, f"{BOLD}Admin:{RESET} .pause, .play, .resetpoint, .reload, .aggressive, .join, .part")
        self.send_private(nick, f"{BOLD}Mode Tim:{RESET} .team create|join|leave|list|start")
        self.send_private(nick, f"{BOLD}In-game:{RESET} jo, od, ti, ca, pl <kode>, a <kode>, cd, tu, dr, d, co <warna>, c <warna>, pa, ct, st")

    def show_version(self):
        self.unomsg(f"{BOLD}{COLOR_BLUE}╔════════════════════════════════╗{RESET}")
        self.unomsg(f"{BOLD}{COLOR_BLUE}║     Uno Bot v3.0 (by Lemon)    ║{RESET}")
        self.unomsg(f"{BOLD}{COLOR_BLUE}╚════════════════════════════════╝{RESET}")
        self.unomsg(f"{COLOR_GREEN}Fitur: mode tim, multi-channel, skor terpisah, bot cerdas & agresif (default), Blackjack bonus, tampilkan kartu pemenang hanya untuk bot, larangan draw jika memiliki Wild Draw Four valid, auto-kick setelah 1 kali idle{RESET}")
    ...
    def _win(self, winner_nick, winning_card=None):
        with self.lock:
            ...
            if self.team_mode:
                ...
                self.unomsg(f"{BOLD}{COLOR_YELLOW}🎉 {COLOR_RED}𝐖𝐈𝐍𝐍𝐄𝐑 🎉{RESET} Tim {winning_team.name} ({', '.join(color_nick(m.nick) for m in winning_team.members)})")
                self.unomsg(f"  {COLOR_GREEN}➤{RESET} Mendapat {winner_points} {UNO_POINTS_NAME}.")

                if winner_points == 21:
                    bonus = 2100
                    winner_points += bonus
                    self.unomsg(f"{BOLD}{COLOR_GREEN}🎰 BLACKJACK! Tim {winning_team.name} mendapatkan bonus {bonus} {UNO_POINTS_NAME}!{RESET}")
                ...
                if winner_nick != self.robot_nick:
                    ...
                    if 3 <= streak <= 15 and streak % 3 == 0:
                        bonus_streak = (streak // 3) * 250
                        self.unomsg(f"{color_nick(winner_nick)} mendapatkan bonus {bonus_streak} {UNO_POINTS_NAME} untuk {streak} kemenangan beruntun!")
                        winner_points += bonus_streak
                ...
                self.unomsg(f"{BOLD}{COLOR_CYAN}📊 STATISTIK GAME 📊{RESET}")
                self.unomsg(f"  ⏱️  Durasi: {duration(time.time()-self.start_time)}")
                self.unomsg(f"  🃏  Kartu dimainkan: {self.cards_played}")
                self._cycle()
                return
    ...
    def _win_normal(self, winner_nick, winning_card=None):
        with self.lock:
            ...
            if winning_card and winner_nick == self.robot_nick:
                self.unomsg(f"{BOLD}{COLOR_YELLOW}🎉 {COLOR_RED}𝐖𝐈𝐍𝐍𝐄𝐑 🎉{RESET} {color_nick(winner_nick)} memainkan {card_display(winning_card)}!")
            else:
                self.unomsg(f"{BOLD}{COLOR_YELLOW}🎉 {COLOR_RED}𝐖𝐈𝐍𝐍𝐄𝐑 🎉{RESET} {color_nick(winner_nick)}")
            ...
            if winner_points == 21:
                bonus = 2100
                winner_points += bonus
                self.unomsg(f"{BOLD}{COLOR_GREEN}🎰 BLACKJACK! {color_nick(winner_nick)} mendapatkan bonus {bonus} {UNO_POINTS_NAME}!{RESET}")
...
def handle_pubmsg(bot, connection, nick, channel, message):
    ...
    if message.startswith('.'):
        ...
        if cmd == 'aggressive':
            ...
            state = "diaktifkan" if game.aggressive_mode else "dinonaktifkan"
        ...
        if cmd == 'version':
            if game:
                ...
                connection.privmsg(channel, f"{UNO_LOGO} Uno Game Poin Minus Dan Mode Tim")
            return
        ...
        if cmd in ('cmds', 'help'):
            if game:
                ...
                connection.notice(nick, "Perintah UNO: .uno .stop .top10 .won [nick] .version .row .pause (admin) .play (admin) .resetpoint (admin) .reload (admin) .aggressive (admin) .join #channel (admin) .part #channel (admin) .remove [nick] .help .cmds. Mode tim: .unotim, .team ...")
            return
