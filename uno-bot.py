...
# Daftar channel yang akan dimasuki bot saat startup
INITIAL_CHANNELS = ["#yobayat"]

UNO_POINTS_NAME = "Poin"         # nama mata uang poin
UNO_SCORE_FILE_TEMPLATE = "UnoScores_{channel}.txt"  # template file skor per channel
UNO_MAX_PLAYERS = 10
UNO_AUTO_SKIP_PERIOD = 30          # detik timeout pemain
UNO_START_GRACE_PERIOD = 30        # detik untuk join
UNO_CYCLE_TIME = 5                  # detik antar game
UNO_ROBOT_RESTART_PERIOD = 1        # jeda bot sebelum bergerak
UNO_DEBUG = 1                       # 1 = tampilkan log debug (opsional)

# Daftar admin (hanya mereka yang bisa .stop, .pause, .play, .resetpoint, .reload, .join, .part)
ADMIN_NICKS = ["Lemon"]
...
def color_nick(nick):
    ...
    return f"\x03{NICK_COLORS[idx]}{nick}{RESET}"
...
def get_score_filename(channel):
    """Mengembalikan nama file skor untuk channel tertentu."""
...
def card_display(card):
    ...
    if val == 'S': return f"{color_code}{BOLD}{color_name} Skip{RESET}"
    if val == 'R': return f"{color_code}{BOLD}{color_name} Reverse{RESET}"
    if val == 'D': return f"{color_code}{BOLD}{color_name} Draw Two{RESET}"
...
class UnoGame:
    def __init__(self, bot, channel):
        ...
        self.is_on = False
        self.mode = 0          # 0:idle,1:join,2:bermain,3:selesai,4:cycle
        self.paused = False
        self.players = []
        self.current_idx = 0
        self.deck = []
    ...
    def _reset_skip_timer(self):
        with self.lock:
            if self.skip_timer: self.skip_timer.cancel()
    ...
    def _schedule_bot_move(self):
        with self.lock:
            if self.bot_timer: self.bot_timer.cancel()
    ...
    def stop(self, nick="console"):
        with self.lock:
            ...
            self.teams = []
            self.team_mode = False
            self.msg(f"Game dihentikan oleh {nick}.")

    def pause_game(self, nick):
        with self.lock:
            if not self.is_on or self.mode != 2: return
            if not self.paused:
                self.paused = True
                self._cancel_skip_timer()
                self.msg(f"Game dijeda oleh {nick}.")
            else:
                self.msg(f"Game sudah dalam keadaan jeda.")
    ...
    def resume_game(self, nick):
        with self.lock:
            ...
            if self.paused:
                ...
                self.msg(f"Game dilanjutkan oleh {nick}.")
            else:
                self.msg(f"Game tidak dalam keadaan jeda.")
    ...
    def join(self, nick):
        with self.lock:
            if self.team_mode:
                ...
                return
            if self.mode != 1 or self.paused: return
            ...
            self.unomsg(f"{color_nick(nick)} bergabung {UNO_LOGO}")
            self.send_private(nick, f"Kartu Anda: {uno_cardcolorall(player.hand)}")

    def join_bot(self):
        with self.lock:
            if any(p.nick == self.robot_nick for p in self.players): return
            ...
            self.unomsg(f"{color_nick(self.robot_nick)} bergabung {UNO_LOGO}")
    ...
    def remove_player(self, nick, by_nick=None):
        with self.lock:
            ...
            if by_nick:
                self.unomsg(f"{color_nick(nick)} telah dikeluarkan dari uno oleh {by_nick}")
            else:
                self.unomsg(f"{color_nick(nick)} meninggalkan Uno")
    ...
    def _begin_team_game(self):
        with self.lock:
            ...
            team_list = "  ".join(f"{t.name}: {', '.join(color_nick(m.nick) for m in t.members)}" for t in self.teams)
            self.unomsg(f"Tim: {team_list}")
            self.unomsg(f"Urutan giliran: " + " → ".join(color_nick(p.nick) for p in self.players))
            self.unomsg(f"Kartu pertama: {card_display(self.top_card)}")
    ...
    # ---------- Logika Game (berlaku untuk kedua mode) ----------
    ...
    def _start_turn(self):
        with self.lock:
            ...
            self.unomsg(f"Giliran: {color_nick(player.nick)}")
            self.is_draw = False
            self._reset_skip_timer()
    ...
    def _current_player(self): return self.players[self.current_idx]

    def _auto_skip(self):
        with self.lock:
            if self.mode != 2 or self.paused: return
            player = self._current_player()
            self.unomsg(f"{color_nick(player.nick)} telah idle dan dilewati")
    ...
    # ---------- Logika Bot (Pintar) ----------
    def _get_opponents_hand_values(self):
        """Mengembalikan dictionary {nick: total_nilai_kartu} untuk semua lawan."""
        values = {}
        for p in self.players:
            if p.nick != self.robot_nick:
                values[p.nick] = p.hand_value()
        return values

    def _get_opponents_near_win(self):
        """Mengembalikan daftar lawan yang hanya memiliki 1 atau 2 kartu."""
        return [p for p in self.players if p.nick != self.robot_nick and p.hand_size() <= 2]

    def _evaluate_card(self, card):
        """Menilai kartu berdasarkan situasi permainan."""
        ct = card_type(card)
        # Skor dasar
        base_scores = {
            'number': 10,
        ...
        # Apakah kita sedang dalam situasi berbahaya (lawan tinggal 1 kartu)
        opponents_near_win = self._get_opponents_near_win()
        for opp in opponents_near_win:
            if opp.hand_size() == 1:
                # Lawan UNO -> gunakan kartu aksi untuk mengganggu
                if ct in ('skip', 'reverse', 'draw2', 'draw4'):
                    score += 1000
            elif opp.hand_size() == 2:
                if ct in ('skip', 'reverse', 'draw2', 'draw4'):
                    score += 500

        # Jika kita sendiri tinggal sedikit kartu, prioritaskan kartu aksi untuk menang lebih cepat
        my_size = self._current_player().hand_size()
        if my_size <= 3:
            if ct in ('skip', 'reverse', 'draw2', 'draw4', 'wild'):
                score += 300
            elif ct == 'number':
                score += 50

        # Pertimbangkan poin kartu: kartu angka tinggi lebih baik dimainkan sekarang
        points = card_points(card)
        if ct == 'number':
            score += points * 2
        elif ct in ('skip', 'reverse', 'draw2'):
            score += points  # 20 poin
        elif ct in ('wild', 'draw4'):
            score += points  # 50 poin

        return score

    def _bot_find_card(self):
        """Memilih kartu terbaik berdasarkan evaluasi."""
        player = self._current_player()
        best_score = -1
        best_index = None

        for i, card in enumerate(player.hand):
            if self._is_valid_play(player, card):
                score = self._evaluate_card(card)
                # Bonus jika ini adalah kartu terakhir (menang)
    ...
    def _bot_pick_color(self):
        """Pilih warna yang paling merugikan lawan."""
        # Hitung warna yang paling banyak di tangan lawan
        opponent_colors = {'R':0, 'G':0, 'B':0, 'Y':0}
        for p in self.players:
            if p.nick != self.robot_nick:
                for card in p.hand:
                    col = card_color(card)
                    if col in opponent_colors:
                        opponent_colors[col] += 1

        # Cari warna yang paling sedikit dimiliki lawan
        if sum(opponent_colors.values()) > 0:
            best_color = min(opponent_colors, key=opponent_colors.get)
        else:
            # Fallback: pilih warna terbanyak di tangan sendiri
            my_colors = {'R':0,'G':0,'B':0,'Y':0}
            for card in self._current_player().hand:
                col = card_color(card)
                if col in my_colors:
                    my_colors[col] += 1
            best_color = max(my_colors, key=my_colors.get)
        return best_color

    def _is_valid_play(self, player, card):
        """Mengembalikan True jika kartu dapat dimainkan oleh player pada keadaan saat ini."""
        ct = card_type(card)
        # Wild Draw Four: hanya bisa dimainkan jika tidak ada kartu dengan warna yang sama di tangan
        if ct == 'draw4':
            for c in player.hand:
                if c == card: continue
                t = card_type(c)
                if t == 'wild' or t == 'draw4':
                    continue
                if card_color(c) == self.current_color:
                    return False
            return True
        if ct == 'wild':
            return True
        if card_color(card) == self.current_color:
            return True
        if card_value(card) == self.current_rank:
            return True
        return False

    def _robot_play(self):
        with self.lock:
            if UNO_DEBUG: print(f"[DEBUG] _robot_play dipanggil")
            if self.mode != 2 or self.paused: return
            player = self._current_player()
            if player.nick != self.robot_nick: return

            if self.is_draw:
                card_index = self._bot_find_card()
                if card_index is not None:
                    card = player.hand[card_index]
                    self.play_card_internal(self.robot_nick, card, from_bot=True)
                else:
    ...
    def play_card_internal(self, nick, card, from_bot=False):
        with self.lock:
            ...
            self.cards_played += 1
            self._cancel_skip_timer()
            self.unomsg(f"{color_nick(nick)} memainkan {card_display(card)}")
    ...
    def _handle_wild_or_draw4(self, nick, card, ct, from_bot):
        with self.lock:
            if from_bot:
                chosen = self._bot_pick_color()
                self.unomsg(f"{color_nick(nick)} memilih warna {COLOR_NAMES[chosen]}.")
    ...
    def _handle_skip(self, player_nick, from_bot):
        with self.lock:
            ...
            victim = self._current_player()
            self._next_player()
            self.unomsg(f"{color_nick(player_nick)} memainkan SKIP! {color_nick(victim.nick)} dilewati.")
            self._show_cards_to_player(self._current_player())
            if self._current_player().nick == self.robot_nick:
                self._schedule_bot_move()
    ...
    def draw_card(self, nick, from_bot=False):
        with self.lock:
            if UNO_DEBUG:
                print(f"[DEBUG] draw_card dipanggil untuk {nick}, from_bot={from_bot}")
            ...
            if not player:
                return
            if UNO_DEBUG:
                print(f"[DEBUG] Sebelum draw, tangan {nick}: {player.hand}")
            ...
            drawn_cards = self._add_draw_to_hand(player, 1)
            if UNO_DEBUG:
                print(f"[DEBUG] Setelah draw, tangan {nick}: {player.hand}")
            self.check_uno(player)
            self.unomsg(f"{color_nick(nick)} mengambil 1 kartu.")
            if not from_bot:
                drawn = drawn_cards[0]
                self.send_private(nick, f"Anda mengambil {card_display(drawn)}")
            else:
                self._schedule_bot_move()
            self._reset_skip_timer()
    ...
    def pass_turn(self, nick):
        with self.lock:
            if UNO_DEBUG:
                print(f"[DEBUG] pass_turn dipanggil untuk {nick}")
            if self.mode != 2 or self.paused: return
            if nick != self._current_player().nick:
                self.notice(nick, "Bukan giliran Anda.")
                return
            ...
            player = self._get_player(nick)
            if UNO_DEBUG:
                print(f"[DEBUG] Sebelum pass, tangan {nick}: {player.hand if player else None}")
            ...
            if self._current_player().nick == self.robot_nick:
                self._schedule_bot_move()
            if UNO_DEBUG:
                print(f"[DEBUG] Setelah pass, tangan {nick}: {player.hand if player else None}")
    ...
    def color_change(self, nick, color):
        with self.lock:
            ...
            self.current_rank = ''
            self.unomsg(f"{color_nick(nick)} memilih warna {COLOR_NAMES[color]}.")
            self.is_color_change = False
            self.color_picker = None
    ...
    def check_uno(self, player):
        if player.hand_size() == 1:
            self.unomsg(f"\001ACTION says {color_nick(player.nick)} UNO!\001")
    ...
    def show_top_card(self): self.unomsg(f"Kartu dalam permainan: {card_display(self.top_card)}")
    def show_turn(self): self.unomsg(f"Current player: {color_nick(self._current_player().nick)}")
    def show_order(self): self.unomsg("Player order: " + "  ".join(color_nick(p.nick) for p in self.players))
    def show_time(self): self.unomsg(f"Game time {duration(time.time()-self.start_time)}")
    def show_card_count(self):
        counts = "  ".join(f"{color_nick(p.nick)}: {p.hand_size()} cards" for p in self.players)
        self.unomsg(counts)
    def show_stats(self): self.unomsg(f"Played: {self.cards_played}")
    def show_help(self, nick):
        self.send_private(nick, "Perintah UNO: .uno .stop .top10 .won .version .row .pause (admin) .play (admin) .resetpoint (admin) .reload (admin) .join #channel (admin) .part #channel (admin) .remove [nick] .help .cmds")
        self.send_private(nick, "Mode tim: .unotim, .team create <nama>, .team join <nama>, .team leave, .team list, .team start")
        self.send_private(nick, "In-game: jo, od, ti, ca, pl <kode>, a <kode>, cd, tu, dr, d, co <warna>, c <warna>, pa, ct, st")
    def show_version(self): self.unomsg("Uno.Tcl (Opsi 2) by Lemon – Ported to Python + mode tim + multi-channel + skor terpisah + bot cerdas")
    ...
    def _random_color(self): return random.choice(['R','G','B','Y'])

    def _win(self, winner_nick):
        with self.lock:
            self.mode = 3
            ...
            if self.team_mode:
                ...
                self.unomsg(f"Tim {winning_team.name} ({', '.join(color_nick(m.nick) for m in winning_team.members)}) menang! Mendapat {winner_points} {UNO_POINTS_NAME}.")
                ...
                if winner_nick != self.robot_nick:
                    ...
                    if 3 <= streak <= 15 and streak % 3 == 0:
                        bonus = (streak // 3) * 250
                        self.unomsg(f"{color_nick(winner_nick)} mendapatkan bonus {bonus} {UNO_POINTS_NAME} untuk {streak} kemenangan beruntun!")
                        winner_points += bonus

                if winner_nick != self.robot_nick:
                    update_score(self.channel, winner_nick, winner_points)
                for team in losing_teams:
                    for p in team.members:
                        if p.nick != self.robot_nick:
                            penalty = p.hand_value() * 2
                            update_score(self.channel, p.nick, -penalty)
                            self.unomsg(f"{color_nick(p.nick)} kalah! Memegang {p.hand_size()} kartu (nilai total {p.hand_value()}), dikurangi {penalty} {UNO_POINTS_NAME}.")
                self.unomsg(f"{self.cards_played} kartu dimainkan dalam {duration(time.time()-self.start_time)}")
                self._cycle()
                return

            self._win_normal(winner_nick)

    def _win_normal(self, winner_nick):
        with self.lock:
            winner = self._get_player(winner_nick)
            total_loser_value = 0
            loser_values = {}
            for p in self.players:
                if p.nick != winner_nick:
                    val = p.hand_value()
                    loser_values[p.nick] = val
                    total_loser_value += val
            winner_points = total_loser_value + 50
            self.unomsg(f"{color_nick(winner_nick)} menang! Mendapat {winner_points} {UNO_POINTS_NAME}.")
            ...
            if winner_nick != self.robot_nick:
                update_score(self.channel, winner_nick, winner_points)
            for nick, val in loser_values.items():
                if nick != self.robot_nick:
                    penalty = val * 2
                    update_score(self.channel, nick, -penalty)
                    self.unomsg(f"{color_nick(nick)} kalah! Memegang {len(self._get_player(nick).hand)} kartu (nilai total {val}), dikurangi {penalty} {UNO_POINTS_NAME}.")
            self.unomsg(f"{self.cards_played} kartu dimainkan dalam {duration(time.time()-self.start_time)}")
            self._cycle()
...
# ==================== DISPATCHER PERINTAH (MULTI-CHANNEL) ====================
games = {}  # key: channel name, value: UnoGame instance
...
def handle_pubmsg(bot, connection, nick, channel, message):
    ...
    # Perintah dengan titik
    if message.startswith('.'):
        parts = message[1:].split()
        cmd = parts[0].lower()
        args = parts[1:]

        # Perintah admin untuk manajemen channel
        if cmd == 'join' and args:
            ...
            connection.join(target_channel)
            connection.privmsg(channel, f"{UNO_LOGO} Mencoba join {target_channel}...")
            return
        ...
        if cmd == 'part' and args:
            if nick not in ADMIN_NICKS:
                ...
                return
            target_channel = args[0]
            if target_channel.lower() == channel.lower():
                connection.notice(nick, "Tidak bisa part dari channel tempat perintah diberikan. Gunakan dari channel lain.")
                return
            connection.part(target_channel, f"Diminta oleh {nick}")
            if target_channel in games:
                if games[target_channel] and games[target_channel].is_on:
                    games[target_channel].stop("console")
                del games[target_channel]
            return
        ...
        if cmd == 'remove':
            ...
            if target != nick and nick not in ADMIN_NICKS:
                connection.notice(nick, "Anda tidak memiliki izin untuk mengeluarkan orang lain.")
                return
            ...
            try:
                if target:
                    ...
                    if target in scores:
                        scores[target] = (0, 0)
                        _write_scores(channel, scores)
                        connection.privmsg(channel, f"{UNO_LOGO} Skor {target} telah di-reset oleh {nick}.")
                    else:
                        connection.notice(nick, f"Pemain {target} tidak ditemukan dalam skor.")
                else:
                    open(get_score_filename(channel), 'w').close()
                    connection.privmsg(channel, f"{UNO_LOGO} Semua skor di channel ini telah di-reset oleh {nick}.")
            except Exception as e:
        ...
        if cmd == 'reload':
            if nick not in ADMIN_NICKS:
                ...
                return
            if game and game.is_on:
                connection.notice(nick, "Tidak bisa reload saat game sedang berjalan. Hentikan game dulu dengan .stop")
                return
        ...
        if cmd == 'version':
            if game:
                ...
                connection.privmsg(channel, f"{UNO_LOGO} Uno.Tcl – Ported to Python + mode tim + multi-channel + skor terpisah + bot cerdas")
            return
        ...
        if cmd in ('cmds', 'help'):
            if game:
                ...
                connection.notice(nick, "Perintah UNO: .uno .stop .top10 .won [nick] .version .row .pause (admin) .play (admin) .resetpoint (admin) .reload (admin) .join #channel (admin) .part #channel (admin) .remove [nick] .help .cmds. Mode tim: .unotim, .team ...")
            return

        # Perintah khusus tim (sub-perintah .team)
        if cmd == 'team':
            ...
            if subcmd == 'create' and len(args) >= 2:
                team_name = args[1]
                game.team_create(nick, team_name)
            elif subcmd == 'join' and len(args) >= 2:
                team_name = args[1]
                game.team_join(nick, team_name)
            elif subcmd == 'leave':
    ...
    # Perintah tanpa titik (hanya jika game aktif)
    if not game or not game.is_on:
        return
    ...
    if cmd == 'jo':
        ...
        game.show_cards(nick)
    elif cmd == 'pl' and args:
        game.play_card(nick, args[0].upper())
    elif cmd == 'a' and args:
        game.play_card(nick, args[0].upper())
    elif cmd == 'cd':
        game.show_top_card()
        ...
        game.draw_card(nick)
    elif cmd == 'co' and args:
        game.color_change(nick, args[0])
    elif cmd == 'c' and args:
        game.color_change(nick, args[0])
    elif cmd == 'pa':
        game.pass_turn(nick)
    ...
    def on_ctcp(self, c, e):
        if e.arguments[0].upper() == 'DCC' and UNO_USE_DCC:
            ...
            if len(parts) >= 3 and parts[0].upper() == 'CHAT':
                ...
                if UNO_DEBUG:
                    print(f"CTCP DCC CHAT from {e.source.nick}, replied with {response}")
        else:
            super().on_ctcp(c, e)
